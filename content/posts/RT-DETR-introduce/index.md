...
DETR首次发表于2020年的论文[《End-to-End Object Detection with Transformers》](https://arxiv.org/abs/2005.12872)，由Nicolas Carion团队发表于CVPR。论文中详细阐述了DETR作为目标检测器的优势和短板，其中最核心的优势就是从根本上摒弃了NMS，完全实现了”端到端”检测。但 DETR 并不是我们今天的主角，因其对多个尺度同时做全局注意力特征计算的特性，模型训练慢，推理极慢，完全不可能用于实时检测领域，感兴趣的读者可以去阅读原论文了解为什么会这样，我们今天将继续模型介绍的这个系列，要介绍的是 DETR 最出名的变种模型之一——RT-DETR。   
...
## RT-DETR详解   
### 背景与定位   
RT-DETR 作为 DETR 的变种，于2023年由百度人工智能研究院提出，研究发表于CVPR2024.论文标题[《DETRs Beat YOLOs on Real-time Object Detection》](https://arxiv.org/abs/2304.08069)，从标题中不难看出，RT 前缀表示 **Real-time**，并直接提出挑战 YOLO 系列在实时检测中的地位。论文核心创新点围绕在：如何在原生不依赖NMS的情况下，让 DETR 类模型达到 YOLO 级别的实时推理速度。接下来，我将详细解析 RT-DETR 如何做到这一点。  

### 网络结构   
#### 1.Backbone  
RT-DETR在backbone选择上采取了相对保守的策略，并没有选取彼时较为火热的 ViT 风格，而是主要选取了已经经过过验证的轻量高效CNN网络——ResNet，对于ResNet各位读者相比已经非常熟悉了，这个backbone现在几乎成为了各路模型默认的baseline，我们不必再过多赘述他的结构优势。   
...
**为什么叫 IoU-aware？** 原始 DETR 仅用分类分数来排序，但分类高并不等价于定位准。RT-DETR 在训练时让模型对每个 query 同时预测分类分数和预测框与真值的 IoU，选取时以两者的乘积作为综合排名依据，从而保证选出的 query 既语义相关又定位可靠，为 Decoder 提供了远高于随机初始化的起点。   
...
两种设计叠加的效果是：模型在 6 个不同精度的预测层上都有直接的梯度监督，既加快了收敛，也改善了对小目标和遮挡目标的检测性能。推理时，只取第 6 层（最后一层）的输出，前 5 层的预测头全部丢弃。
...
讲到目标检测的工程化落地，第一个想到的就是 **Ultralytics**——最早因 YOLOv5、YOLOv8 一战成名，提供了一套极其友好的模型搭建平台。同一套接口，能使用 YOLO 系列、也能使用 SAM、也能跑我们今天的主角 RT-DETR。如果你之前用过 YOLO 系列模型，那 RT-DETR 你已经会用了，几乎零学习成本。
...
## 总结  
本篇介绍了 RT-DETR 的详细原理和网络结构，相信经过刚刚的介绍，各位读者已经能基本搞懂 RT-DETR 的特点了。除此之外，还大概讲了讲如何调用Ultralytics库进行 RT-DETR 的训练，对于没有使用过Ultralytics库的读者也能够快速上手。
